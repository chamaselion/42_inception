...
if [ ! -e /etc/.firstrun ]; then
  if [ -n "$WWW_CONF" ]; then
    sed -i -E 's@^listen\s*=.*@listen = 9000@' "$WWW_CONF" || true
...
for _ in $(seq 1 "$DB_WAIT_TIMEOUT"); do
  if mysql --protocol=tcp --host=mariadb -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" -e "SELECT 1;" >/dev/null 2>&1; then
    DB_READY=1
    break
  fi
  sleep 1
...
if [ ! -f wp-config.php ]; then
  wp config create --allow-root --dbhost=mariadb --dbuser="$MYSQL_USER" --dbpass="$MYSQL_PASSWORD" --dbname="$MYSQL_DATABASE" --path=/var/www/html --force
else
...
if ! wp core is-installed --allow-root --path=/var/www/html >/dev/null 2>&1; then
  wp core install --allow-root --skip-email --url="https://$DOMAIN_NAME" --title="$WORDPRESS_TITLE" --admin_user="$WORDPRESS_ADMIN_USER" --admin_password="$WORDPRESS_ADMIN_PASSWORD" --admin_email="$WORDPRESS_ADMIN_EMAIL" --path=/var/www/html
fi
